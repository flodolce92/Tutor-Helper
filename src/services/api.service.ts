...
class ApiService {
	...
	async getCampuses(): Promise<any[]> {
		const response = await axios.get<any[]>(`${API_BASE}/campus`, {
			headers: this.getHeaders(),
		});
		return response.data;
	}
	...
	// Get users by campus
	async getUsersByCampus(campusId: number, page: number = 1, perPage: number = 100): Promise<UserSearch[]> {
		const response = await axios.get<UserSearch[]>(
			`${API_BASE}/campus/${campusId}/users`,
			{
				headers: this.getHeaders(),
	...
	// Get online users (currently logged in)
	async getOnlineUsers(): Promise<UserSearch[]> {
		const locations = await this.getLocations(1);
		
		// Extract unique user IDs from locations
		const userIds = Array.from(new Set(locations.map(loc => loc.user.id)));
		
		// Fetch user details for each location
